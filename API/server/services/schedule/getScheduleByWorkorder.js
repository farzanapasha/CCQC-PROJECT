...
static getSchedule(id) {
    ...
    .where('schedule.workorder_id', '=', id)
    .select('schedule.id','workorder_id', 'team_id', 'client_id','equipment_id','type_id','status', 'startdate',
    'enddate','schedule.description','equipment.name as equipmentName',"scheduletype.name as typeName","job_name","team.name as teamName").then(data => { 
        return data
   }).catch((err) => {
       console.log(err)
