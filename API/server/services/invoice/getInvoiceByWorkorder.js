...
static getInvoice(id) {
    return DB("invoice")
            ...
            .where('invoice.workorder_id', '=', id)
            .andWhere({ 'invoice.deleted': false })
            .select('invoice.id', 'invoice.status','invoice.createdon',  'workorder.createdon as workordercreatedon', 'payment.createdon as paymentcreatedon', 'workorder_id', 'invoice.client_id',
            'invoice.discount', 'rate', 'tax', "item.name as itemName", "item.id as itemId", 
