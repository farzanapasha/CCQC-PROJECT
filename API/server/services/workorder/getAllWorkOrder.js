...
export default class WorkOrderController {
    static getAll() {
        return DB("workorder")
            ...
            .where('workorder.deleted', '=', false)
            .select('workorder.id', 'workorder.client_id','workorder.createdon',"invoice.id as invoice_id",
            "invoice.createdon as invoicecreatedon","client.name as clientName","client.location as clientLocation",
                'workorder.location', 'workorder.description', 'workorder.type', 'workorder.status_id', 
                'schedule.team_id','schedule.id as schedule_id', "schedule.startdate","schedule.description as scheduleDescription", 
                "schedule.enddate", "team.name","equipment.name as equipmentName", "scheduletype.name as typeName",
                "workorderstatus.name as statusName","invoice.deleted as deletedInvoice").then(data => {
