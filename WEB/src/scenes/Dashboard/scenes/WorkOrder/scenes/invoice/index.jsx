...
return _.find(discountOption,{label:_.find(this.props.item.data,{value:i.item_id}).label})
...

      {
        ...
        disableFilters: true,
        accessor: d =>
          <span>{d.workorder_idDisplay}</span>
      },
      {
        Header: "Payment",
        id: "paymentIds",
        ...
        accessor: d =>
          ...
          >
            {d.paymentIds.length <= 0 ? (
              <div className="schedule-color">
                <i className="fas fa-history"></i>
                <span>create payment</span>
              </div>
            ):<label className="schedule-color">{d.paymentIds[0].idDisplay}</label>}
          </div>
      },
      {
        Header: "Rate",
        id: "rate",
        disableFilters: true,
        accessor:d=><span>{"AED "+numeral(d.rate).format('0,0.00')}</span>
      },
      {
        Header: "Tax",
        id: "tax",
        disableFilters: true,
        accessor:d=><span>{d.tax}%</span>
      },
      {
        Header: "Discount",
        id: "discount",
        disableFilters: true,
        accessor:d=><span>{d.discount}%</span>
      },
      {
        Header: "Paid",
        accessor: "paiedAmount",
        disableFilters: true,
        accessor:d=><span>{"AED "+numeral(d.paiedAmount).format('0,0.00')}</span>
      },
      {
        Header: "Pending Amount",
        id: "pendingAmount",
        disableFilters: true,
        accessor:d=><span>{"AED "+numeral(d.pendingAmount).format('0,0.00')}</span>
    ...
    let itemColumns = [
      {
        ...
        disableFilters: true,
        accessor: d =>
          <span>{_.find(this.props.item.data, { value: d.item_id }).label}</span>
      }, {
        Header: "Quantity",
        accessor: "quantity",
        disableFilters: true
        ...
        id: "amount",
        disableFilters: true,
        accessor:d=><span>{"AED "+numeral(d.amount).format('0,0.00')}</span>
      },
      {
        Header: "sub Total",
        id: "sub Total",
        disableFilters: true,
        accessor:d=><span>{"AED "+numeral((d.amount*d.quantity)).format('0,0.00')}</span>
      },
      {
        Header: "Tax",
        id: "Tax",
        disableFilters: true,
        accessor:d=><span>{"AED "+numeral(((d.amount*d.quantity)*this.state.tax/100)).format('0,0.00')}</span>
      },
      {
        Header: "Grand Total",
        id: "Grand Total",
        disableFilters: true,
        accessor:d=><span>{"AED "+numeral((((d.amount*d.quantity)*this.state.tax/100)+(d.amount*d.quantity))).format('0,0.00')}</span>
      },

       {
        ...
        disableFilters: true,
        accessor: d =>
          <div> {!_.includes(discountsList,_.find(this.props.item.data, { value: d.item_id }).label)&&<i class="fas fa-trash-alt" onClick={(e) => this.deleteItem(d.id)}></i>}</div>
      }]
            ...
            <div className="add-lead-wrapper">
              ...
              <div className="add-lead-form">
                ...
                <div className="invoice-name">
                  <span>Name: {_.find(this.props.client.data,{id:this.props.clientId}).name}</span>  
                </div>
                <div className="invoice-name">
                    <span>Location: {_.find(this.props.client.data,{id:this.props.clientId}).location}</span>
                    </div>
              </div>
              </div>
