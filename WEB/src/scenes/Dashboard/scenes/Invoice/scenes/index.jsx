  ...
  render = () => {
    ...
    const columns = [
      {
        ...
        //       return i.quantity * i.amount
        //     }))
        //     let calculatedDiscount = subTotal * _.toNumber(d.discount) / 100
        //     let calculatedTax = subTotal * _.toNumber(d.tax) / 100

        //     this.setState({
        //       activeInvoiceId: d.id,
        //       "status": d.status,
        //       "workorder_id": d.workorder_id,
        // "client_id": d.client_id,
        //       isStatusDisabled: d.status === 'paid' ? true : false,
        //       "rate": d.rate,
        //       "tax": _.toNumber(d.tax),
      ...
      },
      {
        ...
        Cell: d =>
          <div
            ...
            //     activeInvoiceId: d.id,
            //     activeInvoiceIdDisplay: d.idDisplay,
            //     workorder_id:d.workorder_id
            //   })
          ...
          >
            {d.row.original.paymentIds.length <= 0 ? (
              ...
              //    <i className="fas fa-history"></i> 
                <span>No payments done</span>
              // </div>
            ) : <span >{d.row.original.paymentIds[0].idDisplay}</span>}
          </div>
      },
      {
        Header: "Rate",
    ...
    let itemColumns = [
      ...
      },
      {
        ...
        disableFilters: true,
        Cell: d => <span>{"AED " + numeral((((d.row.original.amount * d.row.original.quantity) * this.state.tax / 100) + (d.row.original.amount * d.row.original.quantity))).format('0,0.00')}</span>
      },
    ...
    let WorkorderOption = _.map(_.filter(this.props.workorder.data, { clientId: this.state.client_id.toString() }), item => {
      return { label: item.workOrderIdDisplay, value: item.workOrderId }
    ...
    if (!this.state.openPaymentModel) {
      return (
        <div className="lead-page-wrapper main">
          <div className="heading-blue-wrapper">
            ...
            {!this.state.openAddInvoice && <div className="tool-component d-none">
              <div>
                <button onClick={(e) => this.setState({
                  openAddInvoice: true, openEditInvoice: false
                })}>
          ...
          <div className="main-space">
            {(!this.state.openAddInvoice && !this.state.openEditInvoice) && <div className="white-bg lead-page-content table-container">
              {this.props.data.data.length > 0 && <Table data={this.props.data.data} columns={columns} enablePagination={true} />}
            </div>}
            ...
            {(this.state.openAddInvoice) &&
              <div className="add-lead-wrapper">
                ...
                <div className="add-lead-form">
                  ...
                  <div> <div className="add-lead-block">
                <label>Client:</label>
                <Select
                  className="select-box"
                  name="client_id"
              ...
              <div className="add-lead-block">
                ...
                <Select
                  className="select-box"
                  name="workorder_id"
                  value={_.find(WorkorderOption, { value: this.state.workorder_id })}
                  onChange={(e) => this.handleSelectChange("workorder_id", e)}
                  options={WorkorderOption}
                />
              </div>}</div>
                  {/* <div>
                <div className="invoice-name">
                  <span>Name: {_.find(this.props.client.data,{id:this.state.clientId}).name}</span>  
                </div>
                <div className="invoice-name">
                    <span>Location: {_.find(this.props.client.data,{id:this.state.clientId}).location}</span>
                    </div>
              </div> */}
                </div>
            ...
            {(this.state.openEditInvoice) &&
              <div className="add-lead-wrapper">
                ...
                <div className="add-lead-form">
                  ...
                  <div>
                    <div className="invoice-name">
                      <span>Name: {_.find(this.props.client.data, { id: this.state.client_id }).name}</span>
                    </div>
                    <div className="invoice-name">
                      <span>Location: {_.find(this.props.client.data, { id: this.state.client_id }).location}</span>
                    </div>
                  </div>
                </div>
      ...
      return (<Payment clientId={this.props.clientId}
        ...
        workOrderId={this.state.workorder_id}
        ...
        paid={_.find(this.props.data.data, { id: this.state.activeInvoiceId }).paiedAmount}
        pending={_.find(this.props.data.data, { id: this.state.activeInvoiceId }).pendingAmount} />)
    }
  };
}
