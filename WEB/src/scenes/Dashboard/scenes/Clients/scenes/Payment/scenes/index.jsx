...
class Home extends Component {
  ...
  deletePayment = (params)=>{
    ...
    .then(({ data }) => {
      ...
      showSuccess:false})
      this.props.getAllPaymentByClient({params:{id:this.props.clientId}})
    }).catch(err=>{
  ...
  handleSelectChange = (name, e) => {
    let amount = 0
    if(name==="invoice_id"){
     amount= _.find(this.props.invoice.clientData,item=>item.id===e.value).pendingAmount
     this.setState({ [name]: e.value,amount:amount });
    }
    else{
  ...
  render = () => {
    ...
    const columns = [
      {
        ...
        accessor: "id",
        Cell: d =>
        <div>{d.row.original.invoiceStatus!=='paid'?<span className="first-name-td" onClick={(e) => {
              this.setState({
      ...
      },
      {
        ...
        Cell: d =>
          <span>{moment(d.row.original.recive_date).format("MMM DD YYYY HH:mm")}</span>
      }, 
      {
        Header: "Edit",
        accessor: "edit",
        disableFilters: true,
        Cell: d =>
          <div> {d.row.original.invoiceStatus!=='paid'?<i class="fas fa-edit" onClick={(e) => {
            this.setState({
            activePaymentId: d.row.original.id,
            "amount":d.row.original.amount,
            "recive_date":moment(d.row.original.recive_date).toDate(),
    ...
    let InvoiceOption =_.map(_.filter(this.props.invoice.clientData,filtitem=>filtitem.status!=='paid'),item=>{
  ...
  const paymodeOption = [{ label: "Cash", value: "cash" },{ label: "Check", value: "check" },{ label: "Credit card", value: "credit card" },{ label: "Other", value: "other" }]
    ...
    return (
      <div>
        {<div className="create-new-payment"><button onClick={(e) => this.setState({ openAddPayment: true,openEditPayment: false
 })}>Create new Payment</button></div>}
