  ...
  deleteInvoice = (params) => {
    request.delete(apiPaths.invoice.deleteinvoice, params)
      .then(({ data }) => {
        ...
        this.props.getAllInvoiceByClient({ params: { id: this.props.clientId } })
      }).catch(err => {
        this.setState({
...
return _.find(discountOption,{label:_.find(this.props.item.data,{value:i.item_id}).label})
    ...
    { label: "10%", value: 10 }]
    let WorkorderOption = _.map(_.filter(this.props.workorder.clientData, { clientId: this.props.clientId.toString()}), item => {
    ...
    if (!this.state.openPaymentModel) {
      return (
        <div>
          ...
          })}>Create new Invoice</button></div>}
          {(!this.state.openAddInvoice && !this.state.openEditInvoice) && <div className="white-bg lead-page-content table-container">
            {this.props.data.clientData.length > 0 && <Table data={this.props.data.clientData} columns={columns} enablePagination={true} />}
          </div>}
      ...
      return (<Payment clientId={this.props.clientId}
        ...
        getAllInvoiceByClient={this.props.getAllInvoiceByClient} 
        paid={_.find(this.props.data.clientData,{id:this.state.activeInvoiceId}).paiedAmount}
        pending={_.find(this.props.data.clientData,{id:this.state.activeInvoiceId}).pendingAmount}/>)
    }
  };
}
